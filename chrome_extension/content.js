...
function injectSummaryPanel() {
  ...
  waitForPageReady().then((ready) => {
    ...
    if (!document.head.querySelector('#youtube-summarizer-styles')) {
      styles.id = 'youtube-summarizer-styles';
    ...
    function attemptInjection() {
      ...
      if (targetContainer) {
        console.log('✅ Found target container:', targetContainer.id || targetContainer.className);
        ...
        retryCount++;
        console.log(`⏳ Target container not found, retrying... (${retryCount}/${maxRetries})`);
        setTimeout(attemptInjection, 200 * retryCount); // Exponential backoff
        return false;
      } else {
        console.log('⚠️ Could not find target container after retries, falling back to body injection');
...
function autoExtractVideoInfo() {
  ...
  if (window.location.hostname.includes('youtube.com') && 
    ...
    function waitForPageReady() {
      return new Promise((resolve) => {
        ...
        function checkReady() {
          ...
          const primary = document.querySelector('#primary');
          const secondary = document.querySelector('#secondary');
          const videoTitle = document.querySelector('h1.ytd-video-primary-info-renderer, h1.style-scope.ytd-watch-metadata');
          
          if ((player || primary) && (secondary || videoTitle)) {
            console.log(`✅ Page ready after ${attempts} attempts`);
            resolve(true);
          } else if (attempts < maxAttempts) {
            setTimeout(checkReady, 250);
          } else {
            console.log(`⚠️ Page ready timeout after ${attempts} attempts`);
            resolve(false);
          }
        }
    ...
    // Wait for page to be ready before extracting info
    waitForPageReady().then(async (ready) => {
      if (!ready) {
        console.log('⚠️ Page not ready, trying to extract anyway...');
      }
      ...
      if (videoInfo) {
        ...
        // Remove old panel first if it exists
        if (summaryPanel) {
          summaryPanel.remove();
          summaryPanel = null;
        }
        
        // Inject the summary panel with delay to ensure DOM is ready
        setTimeout(() => {
          injectSummaryPanel();
          
          // Wait for panel to be injected before updating display
          setTimeout(() => {
            ...
            // Check for existing summaries with additional delay
            setTimeout(async () => {
